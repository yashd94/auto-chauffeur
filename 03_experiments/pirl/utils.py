...
class Memory(object):
    def __init__(self, device, size = 2000, weight = 0.5):
        self.memory = torch.zeros((size, 128)).to(device)
        self.weighted_sum = torch.zeros((size, 128)).to(device)
    ...
    def initialize(self, net, train_loader):
        ...
        for step, batch in enumerate(train_loader):
            with torch.no_grad():                
                ...
                self.weighted_sum[index, :] = output
    ...
    def return_random(self, size, index):
        #t0 = time.time()
        #allowed = [x for x in range(1500) if (x not in index and abs(x - index[0]) > 120)]
        allowed = [x for x in range(index[0] - 180)] + [x for x in range(index[0] + 180, 98280)] #runs much faster
        #t1 = time.time()
...
class ModelCheckpoint():
    ...
    def save_model(self, model, optimizer, current_value, epoch):
        if self.monitor_op(current_value, self.best):
            ...
            torch.save({'model' : model.state_dict(),
                        ...
                        os.path.join(self.directory,'epoch_{}'.format(epoch)))
...
class NoiseContrastiveEstimator():
    def __init__(self, device):
        self.device = device
    def __call__(self, original_features, path_features, index, memory, negative_nb = 4000):   
        loss = 0
        for i in range(original_features.shape[0]): 
            
            temp = 0.07
            #t0 = time.time() 
            cos = torch.nn.CosineSimilarity().to(self.device)
            criterion = torch.nn.CrossEntropyLoss().to(self.device)
            #t1 = time.time()
            negative = memory.return_random(size = negative_nb, index = [index[i]]).detach()
            #t2 = time.time()
