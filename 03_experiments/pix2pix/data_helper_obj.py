...
class LabeledDataset(torch.utils.data.Dataset):    
    def __init__(self, image_folder, annotation_file, scene_index, img_transform, map_transform, extra_info=True):
        """
        Args:
            image_folder (string): the location of the image folder
    ...
    def __getitem__(self, index):
        ...
        obj_path = os.path.join(sample_path, 'obj_map.png')
        obj_map = Image.open(obj_path)
        obj_map = torchvision.transforms.functional.to_tensor(obj_map)
        obj_map = self.map_transform(obj_map)
        ...
        if self.extra_info:
            ...
            extra['obj_map'] = obj_map
            # extra['lane_image'] = lane_image

            return image_tensor, target, obj_map, extra
        
        else:
            return image_tensor, target, obj_map
