...
def draw_filled_box(ax, corners, color):
    point_squence = torch.stack([corners[:, 0], corners[:, 1], corners[:, 3], corners[:, 2], corners[:, 0]])
    xy = torch.Tensor([[(corners[:, 0][0] + corners[:, 1][0]) / 2, (corners[:, 0][1] + corners[:, 1][1]) / 2],
        [(corners[:, 2][0] + corners[:, 3][0]) / 2, (corners[:, 2][1] + corners[:, 3][1]) / 2]])

    xy[0] = xy[0] + (xy[1]-xy[0])*0.22
    xy[1] = xy[1] - (xy[1]-xy[0])*0.22

    # the corners are in meter and time 10 will convert them in pixels
    # Add 400, since the center of the image is at pixel (400, 400)
    # The negative sign is because the y axis is reversed for matplotlib
    lw = math.sqrt((corners[:, 0][0] - corners[:, 1][0])**2 + (corners[:, 0][1] - corners[:, 1][1])**2) * 3.3
    ax.plot(xy.T[0] * 10 + 400, -xy.T[1] * 10 + 400, linewidth = lw, color=color, clip_on = True, markersize=0)
    # ax.plot(Rectanlge)
